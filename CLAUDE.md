...
- Cache v17.
- **23 Sep, table-omission pass (32 parallel subagents, one per chapter):** compared every study page against its own table crop images (not text) for 4 defect classes — dropped tiers, merged dose/arm results, dropped exceptions/qualifiers, overstatement. 21 chapters got real fixes (see `Downloads/omission-pass-2026-09-23.md` for the full per-table log); ch83/88/98 checked clean; 8 chapters (46,47,52,58,60,61,65,99) confirmed to have 0 table crops.
- **New finding: several "0 table crop" chapters actually have tables the build tool never captured as images.** ch47 (Incontinence) has 15 numbered tables transcribed as inline markdown text (not crops); ch60 (BPSD) has 9 tables the same way; ch58 (Delirium) has 5 tables cited by page but with zero crops or text; ch61 (Parkinson) has 7; ch52 (Osteoarthritis) and ch65 (Depression) each have 1. None of these were touched this pass (out of scope — no PDF re-sourcing). Worth a follow-up pass if these tables matter for drilling.
- **New finding: several existing table crops are mis-cropped or mislabeled**, on top of the already-known under-crop bug below — ch42 table42-3 (both pages show only the header row, body cut off), ch44 table44-x_p645 (actually Figure 44-2 + PSG strip, not a table), ch55 table55-5_p828 (shows body prose, not the table), ch59 table59-7_p912 (garbled/overlapping columns), ch63 table63-5_p993 (mislabeled — it's the neuro-syndrome-overview table, not an SCA gene table as a stale citation implied; fixed the citation), ch81 table81-14_p1263 (truncated before Anxiety/Depression/Insomnia rows), ch87 table87-4_p1356 (mislabeled — it's actually a Table 87-3 fragment; real Table 87-4 never cropped) and table87-9 (both pages are TYPE-column slivers, no drug content). None re-cropped this pass (image work, out of scope) — flagging for whoever next touches crop generation.
